...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function generateColor(): ColorRGB {
      // 블루 계열 파스텔톤 (220-260도 범위)
      const hue = 220/360 + Math.random() * (40/360); // 220도~260도 (블루 계열)
      const saturation = 0.3 + Math.random() * 0.3; // 30%~60% (파스텔톤)
      const brightness = 0.7 + Math.random() * 0.3; // 70%~100% (밝은 톤)
